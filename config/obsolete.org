...
<C-M-backspace> 这个按键在系统层面是绑定的 kill xorg 这个命令
#+begin_src emacs-lisp
...
而且我发现自己的笔记本（Linux）用 emacs 的体验比实验室（windows）的体
验好多了，完全是两种体验。不得不说，实验室的电脑实在是有点卡，还不如我
自己的电脑，所以我为什么不带自己电脑去实验室，值得深思的问题？我为什么
要让自己遭罪呢？怎么感觉又是自我设限（汗），真是处处自我设限
...
* Rime
目前还不知道是什么原因，下面这个这行设置不能生效，把这行保留是为了下次
想追究这个原因的方便回忆

这段代码主要是想要配置选中的项的高亮配色
#+begin_src emacs-lisp
...
* VC
快捷键的配置其实没有什么参考价值，等我心理建设一波再把它删除掉
#+begin_src emacs-lisp
